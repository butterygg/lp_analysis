# LP IL-Only Report — `stablecoin_mktcap_mktshare` / `usdt`

_Windows strictly earlier than **2025-09-18**. All figures exclude trading fees._

## Data Source
**Metric which market is forecasting:** USDT market-cap share as of Oct 1, 2025

<details><summary>Oracle Question used to resolve metric value</summary>

Use the DefiLlama stablecoin endpoint at https://stablecoins.llama.fi/stablecoin/1. Extract the circulating supply (pegged USD) series and return the value with the greatest timestamp at or before 2025-10-01 00:00:00 UTC (UTC). For the denominator, rank all USD-pegged stablecoins from https://stablecoins.llama.fi/stablecoins by their circulating peggedUSD value at 2025-10-01 00:00:00 UTC (UTC), select the top 12, and sum those circulating values at that same timestamp.Compute the Tether (USDT) market-cap share as (stablecoin circulating / aggregate circulating) * 100 and report that percentage * 100, rounded up to the nearest integer.
...
## Price Mapping
- Market bounds: **min = 58**, **max = 65**. UP's USD price p is a linear mapping of the metric m into [0,1].
...
- Take m at 60% of range: m = min + 0.60 × (max − min) = 62.2
- UP price: p = (m − min) / (max − min) = (62.2 − 58) / (65 − 58) = **0.600**
...
## Important
- **Mean** -21.17% and **median** -12.85% IL-only returns are shown below.
...
- Count: **697**
- Mean: **-21.17%**, Std: **21.61%**
- Median: **-12.85%**  |  P25: **-34.18%**  |  P10: **-52.72%**  |  P75: **-3.44%**
...
**Example calculation only**: Let's say Merkl shows **200% APY** (this is just an example - actual APY varies by market) and you experience the **median IL loss (-12.85%)**:

1. **Scale Merkl APY to period**: 200% × 0.036 = 7.1%
2. **Convert to multiplier**: 1 + 7.1% = 1.071
3. **Apply median IL loss**: 1.071 × (1 + -12.9%) = 1.071 × 0.871 = 0.934
4. **Net return for 13 days**: -6.6%
5. **Annualized (APY)**: (0.934)^28.1 - 1 = **-85.5% APY**
