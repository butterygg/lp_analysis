# LP IL-Only Report — `stablecoin_mktcap_mktshare` / `usdc`
...
## Price Mapping
- Market bounds: **min = 23**, **max = 30**. UP's USD price p is a linear mapping of the metric m into [0,1].
...
- Take m at 60% of range: m = min + 0.60 × (max − min) = 27.2
- UP price: p = (m − min) / (max − min) = (27.2 − 23) / (30 − 23) = **0.600**
...
## Important
- **Mean** -4.63% and **median** -0.99% IL-only returns are shown below.
...
- Count: **650**
- Mean: **-4.63%**, Std: **10.82%**
- Median: **-0.99%**  |  P25: **-3.13%**  |  P10: **-10.54%**  |  P75: **-0.16%**
...
**Example calculation only**: Let's say Merkl shows **200% APY** (this is just an example - actual APY varies by market) and you experience the **median IL loss (-0.99%)**:

1. **Scale Merkl APY to period**: 200% × 0.036 = 7.1%
2. **Convert to multiplier**: 1 + 7.1% = 1.071
3. **Apply median IL loss**: 1.071 × (1 + -1.0%) = 1.071 × 0.990 = 1.061
4. **Net return for 13 days**: 6.1%
5. **Annualized (APY)**: (1.061)^28.1 - 1 = **422.0% APY**
