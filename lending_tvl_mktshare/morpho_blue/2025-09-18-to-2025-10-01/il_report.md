...
## Price Mapping
- Market bounds: **min = 8.2**, **max = 120**. UP's USD price p is a linear mapping of the metric m into [0,1].
...
- Take m at 60% of range: m = min + 0.60 × (max − min) = 75.28
- UP price: p = (m − min) / (max − min) = (75.28 − 8.2) / (120 − 8.2) = **0.600**
...
## Important
- **Mean** -21.08% and **median** -9.88% IL-only returns are shown below.
...
- Count: **407**
- Mean: **-21.08%**, Std: **23.37%**
- Median: **-9.88%**  |  P25: **-37.21%**  |  P10: **-54.16%**  |  P75: **-3.18%**
...
**Example calculation only**: Let's say Merkl shows **200% APY** (this is just an example - actual APY varies by market) and you experience the **median IL loss (-9.88%)**:

1. **Scale Merkl APY to period**: 200% × 0.036 = 7.1%
2. **Convert to multiplier**: 1 + 7.1% = 1.071
3. **Apply median IL loss**: 1.071 × (1 + -9.9%) = 1.071 × 0.901 = 0.965
4. **Net return for 13 days**: -3.5%
5. **Annualized (APY)**: (0.965)^28.1 - 1 = **-62.8% APY**
