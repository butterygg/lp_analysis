...
## Price Mapping
- Market bounds: **min = 1.4**, **max = 2.8**. UP's USD price p is a linear mapping of the metric m into [0,1].
...
- Take m at 60% of range: m = min + 0.60 × (max − min) = 2.24
- UP price: p = (m − min) / (max − min) = (2.24 − 1.4) / (2.8 − 1.4) = **0.600**
...
## Important
- **Mean** -1.08% and **median** -0.34% IL-only returns are shown below.
...
- Count: **195**
- Mean: **-1.08%**, Std: **1.57%**
- Median: **-0.34%**  |  P25: **-1.36%**  |  P10: **-3.70%**  |  P75: **-0.06%**
...
**Example calculation only**: Let's say Merkl shows **200% APY** (this is just an example - actual APY varies by market) and you experience the **median IL loss (-0.34%)**:
...
5. **Annualized (APY)**: (1.068)^28.1 - 1 = **526.8% APY**
