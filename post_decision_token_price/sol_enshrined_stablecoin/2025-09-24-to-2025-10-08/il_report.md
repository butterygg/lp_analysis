# LP IL-Only Report — `post_decision_token_price` / `sol_enshrined_stablecoin`

_Windows strictly earlier than **2025-09-24**. All figures exclude trading fees._
...
## Price Mapping
- Market bounds: **min = 100**, **max = 300**. UP's USD price p is a linear mapping of the metric m into [0,1].
...
- Take m at 60% of range: m = min + 0.60 × (max − min) = 220
- UP price: p = (m − min) / (max − min) = (220 − 100) / (300 − 100) = **0.600**
...
## Important
- **Mean** -6.07% and **median** -2.03% IL-only returns are shown below.
...
- Count: **616**
- Mean: **-6.07%**, Std: **12.50%**
- Median: **-2.03%**  |  P25: **-6.45%**  |  P10: **-13.07%**  |  P75: **-0.49%**
...
**Example calculation only**: Let's say Merkl shows **200% APY** (this is just an example - actual APY varies by market) and you experience the **median IL loss (-2.03%)**:

1. **Scale Merkl APY to period**: 200% × 0.038 = 7.7%
2. **Convert to multiplier**: 1 + 7.7% = 1.077
3. **Apply median IL loss**: 1.077 × (1 + -2.0%) = 1.077 × 0.980 = 1.055
4. **Net return for 14 days**: 5.5%
5. **Annualized (APY)**: (1.055)^26.1 - 1 = **302.3% APY**
