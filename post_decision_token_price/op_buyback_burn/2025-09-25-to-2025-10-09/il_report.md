# LP IL-Only Report — `post_decision_token_price` / `op_buyback_burn`

_Windows strictly earlier than **2025-09-25**. All figures exclude trading fees._
...
## Price Mapping
- Market bounds: **min = 0.41**, **max = 1.1**. UP's USD price p is a linear mapping of the metric m into [0,1].
...
- Take m at 60% of range: m = min + 0.60 × (max − min) = 0.824
- UP price: p = (m − min) / (max − min) = (0.824 − 0.41) / (1.1 − 0.41) = **0.600**
...
## Important
- **Mean** -7.50% and **median** -1.86% IL-only returns are shown below.
...
- Count: **616**
- Mean: **-7.50%**, Std: **17.26%**
- Median: **-1.86%**  |  P25: **-5.55%**  |  P10: **-17.39%**  |  P75: **-0.36%**
...
**Example calculation only**: Let's say Merkl shows **200% APY** (this is just an example - actual APY varies by market) and you experience the **median IL loss (-1.86%)**:

1. **Scale Merkl APY to period**: 200% × 0.038 = 7.7%
2. **Convert to multiplier**: 1 + 7.7% = 1.077
3. **Apply median IL loss**: 1.077 × (1 + -1.9%) = 1.077 × 0.981 = 1.057
4. **Net return for 14 days**: 5.7%
5. **Annualized (APY)**: (1.057)^26.1 - 1 = **320.9% APY**
