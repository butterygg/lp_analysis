# LP IL-Only Report — `post_decision_token_price` / `drv_supply_increase`
...
## Price Mapping
- Market bounds: **min = 0.015**, **max = 0.15**. UP's USD price p is a linear mapping of the metric m into [0,1].
...
- Take m at 60% of range: m = min + 0.60 × (max − min) = 0.096
- UP price: p = (m − min) / (max − min) = (0.096 − 0.015) / (0.15 − 0.015) = **0.600**
