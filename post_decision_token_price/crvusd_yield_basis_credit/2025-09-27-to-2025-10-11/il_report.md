# LP IL-Only Report — `post_decision_token_price` / `crvusd_yield_basis_credit`

_Windows strictly earlier than **2025-09-27**. All figures exclude trading fees._
...
## Price Mapping
- Market bounds: **min = 0.49**, **max = 1.2**. UP's USD price p is a linear mapping of the metric m into [0,1].
...
- Take m at 60% of range: m = min + 0.60 × (max − min) = 0.916
- UP price: p = (m − min) / (max − min) = (0.916 − 0.49) / (1.2 − 0.49) = **0.600**
...
## Important
- **Mean** -9.44% and **median** -1.51% IL-only returns are shown below.
...
- Count: **616**
- Mean: **-9.44%**, Std: **23.33%**
- Median: **-1.51%**  |  P25: **-4.60%**  |  P10: **-19.08%**  |  P75: **-0.34%**
...
**Example calculation only**: Let's say Merkl shows **200% APY** (this is just an example - actual APY varies by market) and you experience the **median IL loss (-1.51%)**:

1. **Scale Merkl APY to period**: 200% × 0.038 = 7.7%
2. **Convert to multiplier**: 1 + 7.7% = 1.077
3. **Apply median IL loss**: 1.077 × (1 + -1.5%) = 1.077 × 0.985 = 1.060
4. **Net return for 14 days**: 6.0%
5. **Annualized (APY)**: (1.060)^26.1 - 1 = **361.5% APY**
