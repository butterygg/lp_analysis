# LP IL-Only Report — `chain_tvl_mktshare` / `hyperliquid-l1`
...
## Price Mapping
- Market bounds: **min = 1.2**, **max = 2**. UP's USD price p is a linear mapping of the metric m into [0,1].
...
- Take m at 60% of range: m = min + 0.60 × (max − min) = 1.68
- UP price: p = (m − min) / (max − min) = (1.68 − 1.2) / (2 − 1.2) = **0.600**
...
## Important
- **Mean** -10.67% and **median** -7.73% IL-only returns are shown below.
...
- Count: **227**
- Mean: **-10.67%**, Std: **10.43%**
- Median: **-7.73%**  |  P25: **-14.38%**  |  P10: **-25.20%**  |  P75: **-2.17%**
...
**Example calculation only**: Let's say Merkl shows **200% APY** (this is just an example - actual APY varies by market) and you experience the **median IL loss (-7.73%)**:

1. **Scale Merkl APY to period**: 200% × 0.036 = 7.1%
2. **Convert to multiplier**: 1 + 7.1% = 1.071
3. **Apply median IL loss**: 1.071 × (1 + -7.7%) = 1.071 × 0.923 = 0.988
4. **Net return for 13 days**: -1.2%
5. **Annualized (APY)**: (0.988)^28.1 - 1 = **-27.9% APY**
