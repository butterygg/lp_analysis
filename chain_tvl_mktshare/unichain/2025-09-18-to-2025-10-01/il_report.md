...
## Price Mapping
- Market bounds: **min = 0.15**, **max = 0.65**. UP's USD price p is a linear mapping of the metric m into [0,1].
...
- Take m at 60% of range: m = min + 0.60 × (max − min) = 0.45
- UP price: p = (m − min) / (max − min) = (0.45 − 0.15) / (0.65 − 0.15) = **0.600**
...
## Important
- **Mean** -10.48% and **median** -0.41% IL-only returns are shown below.
...
- Count: **148**
- Mean: **-10.48%**, Std: **25.12%**
- Median: **-0.41%**  |  P25: **-0.84%**  |  P10: **-42.75%**  |  P75: **-0.17%**
...
**Example calculation only**: Let's say Merkl shows **200% APY** (this is just an example - actual APY varies by market) and you experience the **median IL loss (-0.41%)**:
...
5. **Annualized (APY)**: (1.067)^28.1 - 1 = **514.4% APY**
