# LP IL-Only Report — `chain_tvl_mktshare` / `unichain`
...
## Price Mapping
- Market bounds: **min = 0.085**, **max = 0.65**. UP's USD price p is a linear mapping of the metric m into [0,1].
...
- Take m at 60% of range: m = min + 0.60 × (max − min) = 0.424
- UP price: p = (m − min) / (max − min) = (0.424 − 0.085) / (0.65 − 0.085) = **0.600**
...
## Important
- **Mean** -9.56% and **median** -9.66% IL-only returns are shown below.
...
- Count: **122**
- Mean: **-9.56%**, Std: **7.27%**
- Median: **-9.66%**  |  P25: **-12.95%**  |  P10: **-22.70%**  |  P75: **-3.03%**
...
**Example calculation only**: Let's say Merkl shows **200% APY** (this is just an example - actual APY varies by market) and you experience the **median IL loss (-9.66%)**:

1. **Scale Merkl APY to period**: 200% × 0.036 = 7.1%
2. **Convert to multiplier**: 1 + 7.1% = 1.071
3. **Apply median IL loss**: 1.071 × (1 + -9.7%) = 1.071 × 0.903 = 0.968
4. **Net return for 13 days**: -3.2%
5. **Annualized (APY)**: (0.968)^28.1 - 1 = **-60.2% APY**
