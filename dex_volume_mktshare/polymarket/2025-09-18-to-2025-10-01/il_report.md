# LP IL-Only Report — `dex_volume_mktshare` / `polymarket`

_Windows strictly earlier than **2025-09-18**. All figures exclude trading fees._

## Data Source
**Metric which market is forecasting:** Polymarket 7-day average volume dominance vs Kalshi as of Oct 1, 2025

<details><summary>Oracle Question used to resolve metric value</summary>

Use the DefiLlama DEX summary endpoint at https://api.llama.fi/summary/dexs/polymarket?excludeTotalDataChart=false&excludeTotalDataChartBreakdown=true. Extract the totalDataChart daily volume series and compute the arithmetic mean of the daily volume values for timestamps t with 2025-10-01 00:00:00 UTC - 7 days ≤ t < 2025-10-01 00:00:00 UTC (UTC). For the denominator, query the same endpoint for each DEX slug in Polymarket (slug polymarket); Kalshi (slug kalshi) and compute their seven-day trailing average daily volume over the same interval. Sum those trailing averages to form the denominator. Compute the volume market share for the DEX identified by dex_slug as (target trailing average / denominator trailing average sum) * 100 and report that percentage as a floating-point value.
...
## Price Mapping
- Market bounds: **min = 0**, **max = 110**. UP's USD price p is a linear mapping of the metric m into [0,1].
...
- Take m at 60% of range: m = min + 0.60 × (max − min) = 66
- UP price: p = (m − min) / (max − min) = (66 − 0) / (110 − 0) = **0.600**
...
## Important
- **Mean** -3.17% and **median** -0.51% IL-only returns are shown below.
...
- Count: **522**
- Mean: **-3.17%**, Std: **9.37%**
- Median: **-0.51%**  |  P25: **-1.94%**  |  P10: **-5.64%**  |  P75: **-0.08%**
...
**Example calculation only**: Let's say Merkl shows **200% APY** (this is just an example - actual APY varies by market) and you experience the **median IL loss (-0.51%)**:

1. **Scale Merkl APY to period**: 200% × 0.036 = 7.1%
2. **Convert to multiplier**: 1 + 7.1% = 1.071
3. **Apply median IL loss**: 1.071 × (1 + -0.5%) = 1.071 × 0.995 = 1.066
4. **Net return for 13 days**: 6.6%
5. **Annualized (APY)**: (1.066)^28.1 - 1 = **497.2% APY**
