...
## Data Source
**Metric which market is forecasting:** BTC/USDT Hyperliquid funding APR - 30-day trailing average as of Sep 26, 2025
...
## Price Mapping
- Market bounds: **min = 0.065**, **max = 40**. UP's USD price p is a linear mapping of the metric m into [0,1].
...
- Take m at 60% of range: m = min + 0.60 × (max − min) = 24.026
- UP price: p = (m − min) / (max − min) = (24.026 − 0.065) / (40 − 0.065) = **0.600**
...
## Important
- **Mean** -13.69% and **median** -7.96% IL-only returns are shown below.
...
- Count: **404**
- Mean: **-13.69%**, Std: **14.53%**
- Median: **-7.96%**  |  P25: **-19.45%**  |  P10: **-37.20%**  |  P75: **-2.69%**
...
**Example calculation only**: Let's say Merkl shows **200% APY** (this is just an example - actual APY varies by market) and you experience the **median IL loss (-7.96%)**:

1. **Scale Merkl APY to period**: 200% × 0.066 = 13.2%
2. **Convert to multiplier**: 1 + 13.2% = 1.132
3. **Apply median IL loss**: 1.132 × (1 + -8.0%) = 1.132 × 0.920 = 1.041
4. **Net return for 24 days**: 4.1%
5. **Annualized (APY)**: (1.041)^15.2 - 1 = **85.3% APY**
