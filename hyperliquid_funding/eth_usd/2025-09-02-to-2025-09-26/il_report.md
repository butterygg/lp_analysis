...
## Price Mapping
- Market bounds: **min = 0**, **max = 45**. UP's USD price p is a linear mapping of the metric m into [0,1].
...
- Take m at 60% of range: m = min + 0.60 × (max − min) = 27
- UP price: p = (m − min) / (max − min) = (27 − 0) / (45 − 0) = **0.600**
...
## Important
- **Mean** -23.43% and **median** -12.02% IL-only returns are shown below.
...
- Count: **404**
- Mean: **-23.43%**, Std: **27.27%**
- Median: **-12.02%**  |  P25: **-32.57%**  |  P10: **-60.73%**  |  P75: **-3.00%**
...
**Example calculation only**: Let's say Merkl shows **200% APY** (this is just an example - actual APY varies by market) and you experience the **median IL loss (-12.02%)**:

1. **Scale Merkl APY to period**: 200% × 0.066 = 13.2%
2. **Convert to multiplier**: 1 + 13.2% = 1.132
3. **Apply median IL loss**: 1.132 × (1 + -12.0%) = 1.132 × 0.880 = 0.995
4. **Net return for 24 days**: -0.5%
5. **Annualized (APY)**: (0.995)^15.2 - 1 = **-6.7% APY**
