...
## Data Source
**Metric which market is forecasting:** USDT Aave supply APY - 30-day trailing average as of Sep 26, 2025
...
## Price Mapping
- Market bounds: **min = 0.55**, **max = 10**. UP's USD price p is a linear mapping of the metric m into [0,1].
...
- Take m at 60% of range: m = min + 0.60 × (max − min) = 6.22
- UP price: p = (m − min) / (max − min) = (6.22 − 0.55) / (10 − 0.55) = **0.600**
...
## Important
- **Mean** -9.61% and **median** -5.76% IL-only returns are shown below.
...
- Count: **374**
- Mean: **-9.61%**, Std: **12.00%**
- Median: **-5.76%**  |  P25: **-13.10%**  |  P10: **-18.53%**  |  P75: **-2.92%**
...
**Example calculation only**: Let's say Merkl shows **200% APY** (this is just an example - actual APY varies by market) and you experience the **median IL loss (-5.76%)**:
...
5. **Annualized (APY)**: (1.066)^15.2 - 1 = **165.6% APY**
