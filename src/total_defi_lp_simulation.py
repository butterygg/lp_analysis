...
class LPPoolSimulator:
    ...
    def simulate_pool_period(
        self,
        tvl_data: TotalTVLData,
        start_timestamp: int,
        end_timestamp: int,
    ) -> Tuple[Dict[int, float], Dict[int, Tuple[float, float]], float]:
        """Simulate UP/DOWN token LP pool for total DeFi TVL over one period."""
        if not tvl_data.total_tvl:
            return {}, {}, 0.0

        available_times = [
            t
            for t in tvl_data.total_tvl.keys()
        ...
        start_tvl = tvl_data.total_tvl[available_times[0]]
        if start_tvl <= 0:
            return {}, {}, 0.0
...
class PortfolioAnalyzer:
    ...
    def calculate_returns(
        ...
        fee_returns = {}
        il_returns = {}
        start_tvl = tvl_data.total_tvl[timestamps[0]]

        for t in timestamps:
            if t in pool_values and t in tvl_data.total_tvl:
                current_tvl = tvl_data.total_tvl[t]
                tvl_ratio = current_tvl / start_tvl
    ...
    def simulate_single_period(
        ...
        total_returns, fee_returns, il_returns = self.calculate_returns(
            pool_values, external_holdings, tvl_data, timestamps, fees
        )

        debug_info = self._calculate_debug_info(tvl_data, timestamps, il_returns)
    ...
    def _calculate_debug_info(
        ...
        start_tvl = tvl_data.total_tvl[timestamps[0]]
        end_tvl = tvl_data.total_tvl[timestamps[-1]]
        start_ratio = 1.0
            ...
            + sum(
                ...
                for ext, price in zip(
                    ...
                    self.pool_simulator.calculate_token_prices(
                        tvl_data.total_tvl[timestamps[-1]]
                        / tvl_data.total_tvl[timestamps[0]]
                    ),
                )
            )
        )
        print(
            f"  Total DeFi: total={final_value:.2f}, return={((final_value/1000-1)*100):.2f}%"
        )
        print(f"  TVL change: {debug_info['tvl_change_pct']:.2f}%")
        print(f"  UP:DOWN ratio change: {debug_info['up_down_ratio_change_pct']:.2f}%")

    def run_portfolio_simulation(
        self, tvl_data: TotalTVLData, start_date: datetime
    ) -> Tuple[
...
class SimulationWorkflow:
    ...
    def run_complete_simulation(self) -> Dict:
        ...
        portfolio_performances, fee_performances, il_performances, debug_info = (
            self.analyzer.run_portfolio_simulation(self.tvl_data, start_date)
        )
