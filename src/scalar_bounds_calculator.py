...
class ScalarBoundsCalculator:
    ...
    def empirical_quantile_bands(self, 
        ...
        if len(log_returns) < 10:
            raise ValueError(f"Insufficient data ({len(log_returns)} periods) for quantile estimation")
    ...
    def bootstrap_confidence_bands(self, 
                                  log_returns: np.ndarray,
                                  confidence_level: float = 0.99,
                                  n_bootstrap: int = 1000,
                                  block_size: int = 5) -> Tuple[float, float]:
        """
        ...
        for _ in range(n_bootstrap):
            ...
            block_indices = np.random.choice(n_returns - block_size + 1, n_blocks, replace=True)
    ...
    def regime_aware_bounds(self,
                           log_returns: np.ndarray,
                           confidence_level: float = 0.99,
                           vol_threshold_percentile: float = 0.75) -> Tuple[float, float]:
        """
        ...
        stress_returns = log_returns[stress_mask] if np.any(stress_mask) else log_returns
        ...
        if len(calm_returns) > 5:
            ...
            calm_q_low, calm_q_high = np.quantile(log_returns, [alpha/2, 1-alpha/2])
        ...
        recent_vol = np.std(log_returns[-window:]) if len(log_returns) > window else np.std(log_returns)
        stress_weight = min(1.0, (recent_vol - vol_threshold) / vol_threshold) if recent_vol > vol_threshold else 0.0
    ...
    def apply_young_series_adjustments(self,
                                      lower_bound: float,
                                      upper_bound: float,
                                      log_returns: np.ndarray) -> Tuple[float, float, Dict]:
        """
        ...
        if data_days < 365:  # Less than 1 year of data
            ...
            if data_days < 180:  # Less than 6 months
                ...
                if len(log_returns) > 0:
                    ...
                    # Weighted average
                    blended_q_low = (1 - prior_weight) * local_q_low + prior_weight * priors['q_low']
                    blended_q_high = (1 - prior_weight) * local_q_high + prior_weight * priors['q_high']
            ...
            if len(log_returns) >= 7:
                ...
                weekly_growth = (current_tvl / week_ago_tvl - 1) if week_ago_tvl > 0 else 0
    ...
    def calculate_bounds(self, confidence_level: float = 0.99) -> ScalarBoundsResult:
        ...
        if len(log_returns) < 10:
            raise ValueError(f"Insufficient data ({len(log_returns)} periods) for bounds calculation")
        
        # Calculate bounds using different methods
        empirical_lower, empirical_upper = self.empirical_quantile_bands(log_returns, confidence_level)
        bootstrap_lower, bootstrap_upper = self.bootstrap_confidence_bands(log_returns, confidence_level)
        regime_lower, regime_upper = self.regime_aware_bounds(log_returns, confidence_level)
    ...
    def visualize_comparison(self, result: ScalarBoundsResult, save_path: str = None) -> None:
        ...
        ax1 = axes[0, 0]
        ax1.plot(filtered_dates, filtered_tvls, 'b-', label=f'{self.chain} TVL', linewidth=2)
        ax1.axhline(result.current_tvl, color='green', linestyle='-', alpha=0.5, label='Current')
        
        # Different bounds methods
        ax1.axhline(result.lower_bound, color='red', linestyle='-', linewidth=2)
        ax1.axhline(result.upper_bound, color='red', linestyle='-', linewidth=2, label='Final (Empirical + Adjustments)')
        
        ax1.axhline(result.bootstrap_lower, color='orange', linestyle='--', alpha=0.5)
        ax1.axhline(result.bootstrap_upper, color='orange', linestyle='--', alpha=0.5, label='Bootstrap')
        
        ax1.axhline(result.regime_aware_lower, color='purple', linestyle=':', alpha=0.5)
        ax1.axhline(result.regime_aware_upper, color='purple', linestyle=':', alpha=0.5, label='Regime-Aware')
        ...
        ax2.axvline(np.mean(log_returns), color='red', linestyle='--', label=f'Mean: {np.mean(log_returns):.3f}')
        ax2.axvline(np.quantile(log_returns, 0.005), color='orange', linestyle='--', label='0.5% quantile')
        ax2.axvline(np.quantile(log_returns, 0.995), color='orange', linestyle='--', label='99.5% quantile')
        ...
        table_data = [
            ...
            ['Empirical + Adj', f'${result.lower_bound/1e6:.1f}M', f'${result.upper_bound/1e6:.1f}M', 
             f'{(result.upper_bound/result.lower_bound):.1f}x'],
            ['Bootstrap', f'${result.bootstrap_lower/1e6:.1f}M', f'${result.bootstrap_upper/1e6:.1f}M',
             f'{(result.bootstrap_upper/result.bootstrap_lower):.1f}x'],
            ['Regime-Aware', f'${result.regime_aware_lower/1e6:.1f}M', f'${result.regime_aware_upper/1e6:.1f}M',
...
- Age Cap Multiplier: {result.age_cap_multiplier or 'N/A'}
        ...
        ax4.text(0.1, 0.5, info_text, fontsize=9, verticalalignment='center', family='monospace')
    ...
    def print_summary(self, result: ScalarBoundsResult) -> None:
        ...
        print(f"  Mean: {result.log_returns_mean:.4f} ({(np.exp(result.log_returns_mean)-1)*100:.2f}% equivalent)")
        print(f"  Std Dev: {result.log_returns_std:.4f}")
        print(f"  {self.period_days}-day Volatility: {result.volatility_30day*100:.1f}%")
        print(f"  Annualized Volatility: {result.volatility_annual*100:.1f}%")
        
        print(f"\nFINAL BOUNDS ({result.confidence_level:.0%} confidence):")
        print(f"  Lower: ${result.lower_bound/1e9:.2f}B ({(result.lower_bound/result.current_tvl-1)*100:+.1f}%)")
        print(f"  Upper: ${result.upper_bound/1e9:.2f}B ({(result.upper_bound/result.current_tvl-1)*100:+.1f}%)")
        print(f"  Ratio: {result.upper_bound/result.lower_bound:.1f}x")
        
        print(f"\nAlternative Methods:")
        print(f"  Bootstrap:    [${result.bootstrap_lower/1e9:.2f}B, ${result.bootstrap_upper/1e9:.2f}B]")
        print(f"  Regime-Aware: [${result.regime_aware_lower/1e9:.2f}B, ${result.regime_aware_upper/1e9:.2f}B]")
...
def main(chain: str = "Base", period_days: int = 30):
    ...
    calculator = ScalarBoundsCalculator(chain=chain, lookback_months=12, period_days=period_days)
