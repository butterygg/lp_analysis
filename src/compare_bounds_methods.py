...
def compare_chains():
    ...
    for chain in chains:
        ...
        if 'old' in results:
            ...
            print(f"  Range:   [{(lower/current-1)*100:+.1f}%, {(upper/current-1)*100:+.1f}%]")
        ...
        if 'new' in results:
            ...
            print(f"  Bootstrap:    [${new['bootstrap_lower']/1e9:.2f}B, ${new['bootstrap_upper']/1e9:.2f}B]")
            print(f"  Regime-Aware: [${new['regime_aware_lower']/1e9:.2f}B, ${new['regime_aware_upper']/1e9:.2f}B]")
...
""")
    ...
    for i, chain in enumerate(chains):
        ...
        if 'old' in results and 'new' in results:
            ...
            # Current value line
            ax.axhline(current, color='green', linestyle='--', alpha=0.5, label=f'Current: ${current:.1f}B')
